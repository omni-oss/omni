...
describe(
    "bridge_rpc_services – throughput (omni_bridge_test_service)",
    {
        timeout: 20_000,
        concurrent: false,
    },
    () => {
        ...
        describe("proc services", () => {
            it("handles 100 concurrent refreshSnapshot calls without errors", async () => {
                ...
                expect(Object.keys(system.proc.env()).length).toBeGreaterThan(
                    0,
                );
            });
        ...
        describe("fs services", () => {
            it("handles 50 concurrent file write + read pairs without corruption", async () => {
                ...
                for (const [i, text] of texts.entries()) {
                    ...
                    const expectedLength = `file-${i}: ${"x".repeat(512)}`
                        .length;
            ...
            it("handles 100 concurrent /fs/path-exists queries without misrouting", async () => {
                ...
                await Promise.all(
                    existingFiles.map((f) =>
                        system.fs.writeStringToFile(f, "x"),
                    ),
                );
