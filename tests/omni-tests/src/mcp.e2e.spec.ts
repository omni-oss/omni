...
describe(
    ...
    () => {
        ...
        it("cache_dir is nested inside root_dir", async () => {
            ...
            expect(normalize(data.cache_dir)).toContain(
                normalize(data.root_dir),
            );
        });
...
describe(
    ...
    () => {
        it("dry_run=true reports actions without writing files", async () => {
            ...
            const data = getContent<{ ok: boolean; actions: unknown[] }>(
                result,
            );
...
describe(
    "+mcp @mcp @cli (generator_run parallelism)",
    {
        tags: ["mcp", "generator"],
    },
    () => {
...
describe(
    "+mcp @mcp @cli (generator_run recursion)",
    {
        tags: ["mcp", "generator"],
    },
    () => {
...
describe(
    "+mcp @mcp @cli (generator_run max_depth)",
    {
        tags: ["mcp", "generator"],
    },
    () => {
...
describe(
    "+mcp @mcp @cli (generator_validate_input)",
    {
    ...
    () => {
        it("valid=true when an explicit value is provided for the required input", async () => {
            ...
            const data = getContent<{ valid: boolean; errors: unknown[] }>(
                result,
            );
        ...
        it("flags a required conditional input as missing when its gate is active", async () => {
            ...
            expect(
                data.errors.some((e) => e.input_name === "crate_version"),
            ).toBe(true);
            // Must be a per-field error, not a config-level rendering failure.
            expect(
                data.errors.some((e) => e.input_name === "_configuration"),
            ).toBe(false);
        });
...
describe(
    ...
    () => {
        ...
        it("reflects entries after tasks have been run", async () => {
            ...
            const appProject = data.projects.find(
                (p) => p.project_name === "app",
            );
...
describe(
    "+mcp @mcp @cli (task_run include_logs)",
    {
...
// whose availability and quote-handling vary across shells.
describe(
    "+mcp @mcp @cli (exec_command include_logs)",
    {
        tags: ["mcp", "output"],
    },
    () => {
        ...
        it("captures a failing command's output under the default (failed) policy", async () => {
            ...
            expect(
                data.results.some((r) =>
                    (r.logs ?? "").includes("EXEC-FAIL-MARK"),
                ),
            ).toBe(true);
        });
        ...
        it("omits a successful command's output under the default (failed) policy", async () => {
            ...
            expect(data.ok).toBe(true);
            expect(data.results.every((r) => (r.logs ?? null) === null)).toBe(
                true,
            );
        });
    },
...
describe(
    "+mcp @mcp @cli (task_run cached logs)",
    {
