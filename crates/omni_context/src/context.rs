...
use crate::{
    ContextSys, Error as ContextError, ErrorInner,
    constants::{self},
};
...
use omni_core::{ExtensionGraph, ExtensionGraphNode, Project, Task};
...
pub fn get_root_dir(sys: &impl ContextSys) -> Result<PathBuf, ContextError> {
    ...
    Err(ErrorInner::FailedToFindWorkspaceConfiguration.into())
}
...
fn get_workspace_configuration(
    ...
    let ws_path =
        ws_path.ok_or(ErrorInner::FailedToFindWorkspaceConfiguration)?;

    let f = sys.fs_read(&ws_path)?;
    let w =
        serde_yml::from_slice::<WorkspaceConfiguration>(&f).map_err(|e| {
            ErrorInner::FailedToLoadWorkspaceConfiguration(ws_path.clone(), e)
        })?;
