...
//!    matching the cross-platform contract. Keep `net` out of the *claimed*
//!    coverage (host-level net enforcement stays with the shim/broker). The
//!    Linux backend does lower a partial net floor (Landlock V4's port-only
//!    `connect_ports`), but AppContainer has no equivalent to mirror: its net
//!    control is a coarse capability-SID on/off switch (`internetClient` and
//!    friends) that cannot express `host:port`, so there is no precise net floor
//!    to lower here.
