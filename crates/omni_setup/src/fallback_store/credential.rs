...
impl CredKey {
    ...
    pub fn with_unique_pair<T, F>(&self, f: F) -> keyring_core::Result<T>
    where
                ...
                match self.store.creds.get(&self.id) {
                    ...
                    Some(pair) => {
                        ...
                        match creds.len() {
                            ...
                            0 => Err(keyring_core::Error::NoEntry),
                            // just one current cred, get it
                            1 => {
                                let mut first =
                                    creds.iter_mut().next().unwrap();
                                let (key, val) = first.pair_mut();
                                Ok(f(key, val))
                            }
                            // more than one cred - ambiguous!
                            _ => {
                                let mut entries: Vec<keyring_core::Entry> =
                                    vec![];
                                for cred in creds.iter() {
                                    ...
                                    entries.push(keyring_core::Entry::new_with_credential(
                                        Arc::new(key),
                                    ));
                                }
                                Err(keyring_core::Error::Ambiguous(entries))
                            }
                        }
                    }
                }
    ...
    pub fn with_unique_cred<T, F>(&self, f: F) -> keyring_core::Result<T>
    where
    ...
    pub fn get_uuid(&self) -> keyring_core::Result<String> {
        self.with_unique_pair(|uuid, _| uuid.to_string())
    }

    /// This returns the comment of the sole credential for this cred.
    #[allow(unused)]
    pub fn get_comment(&self) -> keyring_core::Result<Option<String>> {
...
impl CredentialApi for CredKey {
    ...
    fn set_secret(&self, secret: &[u8]) -> keyring_core::Result<()> {
        let result =
            self.with_unique_cred(|cred| cred.secret = secret.to_vec());
        match result {
            Ok(_) => Ok(()),
            // a specifier with no credential: create the cred
            Err(keyring_core::Error::NoEntry) if self.uuid.is_none() => {
    ...
    /// and `creation-date`.
    fn get_attributes(&self) -> keyring_core::Result<HashMap<String, String>> {
    ...
    fn update_attributes(
        ...
        keyring_core::attributes::parse_attributes(&["comment"], Some(attrs))?;
