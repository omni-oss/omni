        ...
        CredValue {
            ...
            creation_date: Some(
                OffsetDateTime::now_utc()
                    ...
                    .expect("should be able to format time"),
            ),
        }
    }
}
...
pub struct Store {
    pub id: String,
    pub creds: CredMap,
    pub backing: Option<PathBuf>, // the backing file, if any
...
impl Drop for Store {
    fn drop(&mut self) {
        if self.backing.is_none() {
            log::debug!("dropping store {self:?}")
        } else {
            log::debug!("Saving store {self:?} on drop...");
            match self.save() {
                Ok(_) => log::debug!("Save of store {self:?} completed"),
                Err(e) => log::error!("Save of store {self:?} failed: {e:?}"),
            }
        }
    }
}
...
impl Store {
    ...
    #[allow(unused)]
    pub fn new_with_backing(path: &Path) -> Result<Arc<Self>> {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let content = rmp_serde::to_vec(&self.creds)
    ...
    pub fn new_internal(creds: CredMap, backing: Option<PathBuf>) -> Arc<Self> {
        let store = Store {
            id: format!(
        ...
        log::debug!("Created new store: {store:?}");
    ...
    pub fn load_credentials(path: &Path) -> Result<CredMap> {
        match std::fs::exists(path) {
            Ok(true) => match std::fs::read(path) {
                Ok(s) => Ok(rmp_serde::from_slice(&s)
                    .map_err(|e| PlatformFailure(Box::from(e)))?),
...
impl CredentialStoreApi for Store {
    ...
    fn build(
        ...
        if let Some(force_create) =
            parse_attributes(&["force-create"], mods)?.get("force-create")
        {
        ...
        Ok(Entry::new_with_credential(Arc::new(key)))
    }
    ...
    fn search(&self, spec: &HashMap<&str, &str>) -> Result<Vec<Entry>> {
        ...
        for pair in self.creds.iter() {
            ...
            for cred in pair.value().iter() {
                ...
                result.push(Entry::new_with_credential(Arc::new(CredKey {
                    store: store.clone(),
                    id: pair.key().clone(),
