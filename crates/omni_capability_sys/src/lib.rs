...
mod tests {
    ...
    use system_traits::{
        BaseFsGlobAsync, BaseFsReadAsync as _, BaseFsReadDirAsync,
        EnvSnapshot as _, FsCreateDirAllAsync as _, FsGlobAsync as _,
        FsReadAsync as _, FsReadDirAsync as _, FsWriteAsync as _,
    };
    ...
    /// A minimal `BaseFsReadDirAsync` handle returning a fixed entry set, so the
    /// decorator's directory-listing gating can be tested in isolation.
    struct FakeReadDir(Vec<std::path::PathBuf>);
    #[async_trait::async_trait]
    impl BaseFsReadDirAsync for FakeReadDir {
    ...
    #[tokio::test]
    async fn read_dir_filters_out_denied_entries() {
        // Listing an allowed directory must not disclose the names of entries the
        // policy denies reading — the same rule `base_fs_glob_async` applies.
        let inner = FakeReadDir(vec![
            "/repo/a.txt".into(),
            "/repo/nested".into(),
            "/repo/secret".into(),
        ]);
        // `RepoOnly` allows everything under `/repo`; tighten to a stricter
        // authorizer so a denied sibling is present to be filtered.
        let sys = PolicyEnforcingSys::new(inner, RepoSubtree("/repo/secret"));

        let entries = sys.fs_read_dir_async("/repo").await.unwrap();
        assert_eq!(
            entries,
            vec![
                std::path::PathBuf::from("/repo/a.txt"),
                std::path::PathBuf::from("/repo/nested"),
            ],
            "a denied child must not appear in the listing"
        );
    }

    /// Allows reads/writes under `/repo` **except** a single denied subtree,
    /// proving `read_dir` omits a denied entry while keeping its allowed siblings.
    struct RepoSubtree(&'static str);
    impl CapabilityAuthorizer for RepoSubtree {
        fn authorize(&self, request: &Request<'_>) -> Decision {
            let allowed = matches!(
                request,
                Request::Fs { path, .. }
                    if path.starts_with("/repo") && !path.starts_with(self.0)
            );
            if allowed {
                Decision::Allow
            } else {
