...
export async function applyVersion(args?: ApplyVersionArgs) {
    ...
    const files = await walk({
        ...
        includeEmpty: false,
        path: args?.startDir ?? process.cwd(),
        follow: false,
    });

    const FILES_TO_UPDATE = args?.filesToUpdate ?? [
        "package.json",
        "Cargo.toml",
    ];
    const REPLACEMENT_PATTERN =
        args?.versionRegex ?? /0.0.0-semantically-released/g;
    ...
    if (
        await Bun.file(args?.versionFile ?? `${currentDir}/.version`).exists()
    ) {
